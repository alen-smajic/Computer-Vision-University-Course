...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # values for later. The generated output stores the scores of every sample for the different classes.
        ...
        # this we use the predefined function softmax_loss. The function also outputs the gradient of the loss function 
        ...
        # we get an increased loss value (if regularization is used and the self.reg value is non-zero).
        reg_w1 = np.sum(np.square(self.params['W1']))
        reg_w2 = np.sum(np.square(self.params['W2']))
        loss += self.reg * 0.5 * (reg_w1+reg_w2)
        
        # Now we want to calculate our gradients with respect to the weights which connect the hidden and the output layer. To do this
        ...
        # hypterparameter self_reg (since the square is cancelled out by the 0.5 value). We take these values and add it 
        # to the other gradient values to get the correct gradient. The bias is unaffected since it was not regularized.
        dw_2 += self.reg*self.params['W2']
        ...
        # layer. Since we used the affine_relu_forward function in the forward pass, we need to use the 
        ...
        # We also need to computee again the gradient regarding the regularization term. The same explanation from above
        # applys here too.
        dw_1 += self.reg*self.params['W1']
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # had all information which we needed in the form of the num_layers, input_dim, num_classes and hidden_dims variables. The
        ...
        # of our possible layers. The important aspect here is that every weight W has dimension (previous_layer x next_layer) except the first
        # and the last layer. The first layer has dimension (input_dim x hidden_dims[0]) and the last layer has dimension 
        # (hidden_dims[-1] x num_classes). We use the variable current_dim to store the dimension of the current layer in order to use it 
    ...
    def loss(self, X, y=None):
        ...
        # way to do this is a dictionary in which we store the information for every layer.
        caches = {}
        ...
        for i in range(self.num_layers-1):
            ...
            if self.use_dropout:                
                # If we are using dropout the following two code cells will be executed. They basically take the output
                # from the hidden layers, after ReLu was applied, and apply the dropout_forward function using the 
                # dropout_param as the dropout parameter. After we dropped some of the neurons, we store the cache values
                # for the latter backpropagation process and continue with the remaining layers. It is important to say that
                # dropout is not applied on the output layer.
                output, do_cache = dropout_forward(output, self.dropout_param)
                caches.update({'do_cache{0}'.format(i+1): do_cache})
        ...
        # self.reg with our weight parameters and add the values to the corresponding gradient values.
        ...
        # Lastly, we loop through our other layers to calculate the other gradients similarly to the
        # two-layer neural network. To calculate the local gradients we use the affine_relu_backward
        # function (since we used the corresponding forward version in the forward pass). Furthermore,
        # we have to calculate the gradient of the regularization for every iteration and add these
        # values to the other gradient values. 
        for i in range(self.num_layers-1, 0, -1):
            if self.use_dropout:
                # If we used dropout for the forward pass, we need to derive it also for the backward 
                # pass. Since it was applied after the ReLu function, it is basically the last operation
                # which is applied to a value inside a layer. Because of that it is the first operation
                # which we need to consider when calculating the backward pass (for a hidden layer). To 
                # calculate it, we simply use the dropout_backward function and provide it with the upstream 
                # derivative and the cache values which we stored when we applied the corresponding forward
                # pass. After we get the result dx we just forward it to the next operation which is the 
                # affine_relu_backward and continue as usual.
                dx = dropout_backward(dx, caches['do_cache'+str(i)])
